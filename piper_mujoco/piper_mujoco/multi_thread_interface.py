...
class MujocoROSBridge(Node):
    ...
    def __init__(self, robot_info):
        ...
        self.moveit_joint_names.append("joint7")
        self.moveit_joint_names.append("joint8")
        for i in range(1, 7):
            self.moveit_joint_names.append(f"joint{i}")

        # ROS2 Subscriber 설정 - MoveIt이 만든 Trajectory 받기
        self.latest_joint_set = None
        self.joint_set_mutex = threading.Lock()
        self.joint_set_sub = self.create_subscription(JointState, '/piper/joint_set', self.joint_set_callback, 10)
        # <----------------- 윗 부분 추가 ------------------------>
    ...
    def joint_publish_callback(self):
        ...
        js_msg.position = [float(self.data.qpos[6]),float(self.data.qpos[7])] + [float(self.data.qpos[i]) for i in range(self.ctrl_dof-2)]
    ...
    def run(self):        
        ...
        try:     
            with mj_view.launch_passive(self.model, self.data) as viewer:    
                ...
                d_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_NUMERIC, "init_cam_distance")
                a_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_NUMERIC, "init_cam_azimuth")
                e_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_NUMERIC, "init_cam_elevation")

                viewer.cam.distance = self.model.numeric_data[d_id]
                viewer.cam.azimuth = self.model.numeric_data[a_id]
                viewer.cam.elevation = self.model.numeric_data[e_id]

                # cam_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_CAMERA, 'omniscient_view')
